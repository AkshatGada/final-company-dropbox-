...
class App extends Component {
  ...
  async loadBlockchainData() {
    if (window.ethereum) {
      try {
        ...
        const address = "0xc4749c9bEDff98caA5E5cd7AD4720dB706B8B388"; //repalce dstorage contract address here

        console.log(Dstorage.abi, "abi");
        ...
        console.log(marketplace, "marktetplace");
  ...
  async checkNft() {
    const owner = this.state.marketplace?.filters.Bought(
      null,
      "0x35AFB3c0d77217f4527b3ACa2832a9Bb0d3153bF", //replace nft contract address here
      null,
      null,
      null,
      null
    ...
    // const res = await this.state.provider.getLogs(owner);
    const res = await this.state.marketplace?.queryFilter(owner, 1, "latest");
    console.log(res, "res");

    this.setState({ array: res });
    console.log(this.state.array, "arr");
  ...
  auth() {
    ...
    for (let i = 0; i < this.state.array.length; i++) {
      console.log(this.state.array[i].args.buyer.toLowerCase(), "blk");
      // console.log(this.state.account.toLowerCase(), "own");

      let auth =
        this.state.array[i].args.buyer.toLowerCase() ===
  ...
  async check() {
    ...
    const num = await this.state.dstorage.fileCount();
    console.log(num, "num");
    const count = await this.state.dstorage.files(num);
  ...
  render() {
    return this.state.auth && !this.state.authenticate ? (
      <div>
        <section class="text-gray-600 body-font">
          ...
          <img alt="content" class="object-cover object-center h-full w-full" src="https://dummyimage.com/1201x501"/>
                ...
                <button
                  class="flex mx-auto mt-6 text-black bg-indigo-1000 border-0 py-2 px-5 focus:outline-none hover:bg-indigo-600 rounded"
                  onClick={() => this.loadeverythingup()}
                >
                  Connect your Wallet
                </button>
                <div>
          ...
          <div class="container mx-auto flex px-5 py-24 md:flex-row flex-col items-center">
            <div class="lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
              ...
              <p class="mb-8 leading-relaxed">
                Decentralized storage refers to a system where data is stored
                across a network of computers rather than in a centralized
                location. It offers improved data security, privacy, and
                reliability by distributing data across multiple nodes. Users
                can access their data from any node, and no single point of
                failure exists. It promotes a more resilient and
                censorship-resistant approach to data storage and retrieval.
              </p>
              <div class="flex justify-center">
                <button
                  class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow"
                  onClick={() => this.getFileHash()}
                >
                  HASH
                </button>
                <button
